...
revisions_path = os.path.join(data_dir, "other_outputs", "finegrained", "pro_armenian_total.csv")
...
# Copy to new DataFrame and use the first rows for testing
revisions_new = revisions.copy()[["source", "finegrained_judgment", "finegrained_analysis"]]
...
with ThreadPoolExecutor(max_workers=4) as executor:
    results = list(tqdm(executor.map(process_text, revisions_new['finegrained_analysis']), total=len(revisions_new)))

# Assign back to DataFrame
revisions_new['weaponization_technique'] = results

output_directory = os.path.join(data_dir, "other_outputs", "finegrained", "weaponization_analysis")
os.makedirs(output_directory, exist_ok=True)
output_path = os.path.join(output_directory, "pro-armenian_all_revisions_with_weaponization_techniques.csv")
revisions_new.to_csv(output_path, index=False)
